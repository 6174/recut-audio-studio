 ...
 * [OUTPUT]: 对外提供运行状态、素材、转写、角色与合成输出的 UI 类型
 * [POS]: ui/src 的领域契约；组件不重复解释后端记录字段
 * [PROTOCOL]: 变更时更新此头部，然后检查 README.md
 */
export type WhisperModel = "whisper-small" | "whisper-medium" | "whisper-large-v3";
...
export type RuntimeStatus = { ready: boolean; pending?: boolean; modelsRoot: string; error?: string; asr: { installed: WhisperModel[] }; tts: { ready: boolean; repository?: boolean; model?: boolean }; activeJob?: ActiveAudioJob | null; setupError?: string; setupLogs?: ShellJobLog[] };
...
export type TranscriptSummary = { id: string; sourceAssetId: string; sourceKind: SourceKind; model: WhisperModel; language: string; duration: number; createdAt: string; srtURL: string; jsonURL: string };
export type TranscriptDetail = TranscriptSummary & { segments: TranscriptSegment[]; srt: string; status?: string; error?: string };
export type VoiceCharacter = { id: string; name: string; model: WhisperModel; promptText: string; sampleAssetId: string; createdAt: string; sampleURL: string };
