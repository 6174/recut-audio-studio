...
成员清单
audio_runner.py: 本机 Python 执行入口；检查 FFmpeg 与模型状态、按文件下载 Whisper / CosyVoice2 权重、抽取音频轨道并转写为 transcript.json 文稿与 SRT 字幕、把参考音归一化为 16k 单声道 wav 并自动生成角色提示词、用 CosyVoice2 零样本或情绪指令合成配音 wav，各阶段输出实时进度；不管理 venv 或依赖安装。
