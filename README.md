...
- **结果先私有，确认后再入库**：转写与配音先保留在 App 私有文件区，用户明确保存后才创建素材库 Asset；AI 创建的声音角色默认直接入库（人工 UI 路径默认不入库，可勾选或事后保存）。
...
1. **准备环境**：首次打开自动准备依赖；之后的模型下载与环境安装都在右上角「模型与环境」设置面板中按需完成（Hugging Face / ModelScope / 自动回退）。
2. **转写**：选择素材库中的音频或视频、模型与语言，生成文稿与 SRT，满意后保存为 transcript 素材。
3. **创建声音角色**：三个入口——上传一段人声素材克隆、用 VoxCPM2 Voice Design 描述设计、或管理已有角色（试听/保存/删除）；也可以从 20 个内置预设选一个，或用一段中文描述设计声音（`audio.character.design`）。
...
| **任务中心** | 统一查看、检索与取消所有任务（含排队中），查看持久化日志；推理任务单槽排队、下载可并行 | `audio.tasks.list` · `audio.task.get` · `audio.task.logs` · `audio.task.cancel` |
...
- **模型与环境设置（右上角齿轮）**：集中管理三个运行环境（主 / CosyVoice / VoxCPM 专属 venv）、全部模型（Whisper / Qwen3-ASR / CosyVoice2 / VoxCPM 三档）、下载源与声音预设缓存状态；工作流步骤内只保留就绪状态与「打开设置」引导。
- **转写**：选择素材与语言，查看分段文稿与 SRT 预览，保存为 transcript 素材。
- **声音角色**：一级模态框三个入口——上传参考音克隆、用 VoxCPM 设计声音、管理已有角色（试听/保存/删除，含来源徽标）。
...
**为什么不能同时跑多个任务？** 推理类任务（转写/建角色/设计声音/配音）都在同一块 GPU 上加载大模型，并行会 OOM，因此统一单槽串行：同时发起多个时，后来的会进入「排队中」并在前面的任务结束后自动开始（不会报错拒绝，也不需手动重发）。模型下载（`audio.install`）与推理互不占资源，可随时并行发起。入口卡片与任务中心会分别标注每个功能的「进行中/排队中」状态。

**转写或配音按钮不可用？** 可能是该功能已有排队/进行中的任务（每功能限一单）；或依赖未就绪——打开右上角「模型与环境」设置面板查看哪项未就绪，按行按钮安装；也可用 `audio.status` 检查 Python、FFmpeg 与模型状态，定向修复某个专属 venv 用 `audio.prepare { target: "cosyvoice" | "voxcpm" }`。

**声音角色创建失败？** 参考音需包含清晰、连续的人声片段；系统会验证波形与声纹，未通过验收的输入不会创建角色，建议更换更干净的人声素材重试。

**VoxCPM 模型很大，下载失败怎么办？** 在「模型与环境」设置面板中切换 Hugging Face / ModelScope / 自动回退来源后重新下载；VoxCPM 首次使用会先安装独立 venv，日志可在任务中心查看。
