...
1. **转写**：选择素材库里的音频或视频，在本机用 faster-whisper 转写，产出可编辑文稿（`transcript.json`）和 SRT 字幕。
2. **声音角色**：上传一段 5~15 秒干净人声，App 抽取 16k 参考音并用 Whisper 自动生成角色提示词，形成可复用的声音角色。
...
2. **转写**：选择 Small / Medium / Large 之一下载 Whisper 模型（可多选），选择音频或视频素材与语言，生成后先查看文稿与字幕，再按需保存或复制。
3. **声音角色**：确认已安装 Whisper 模型，从素材库选择一段人声音频，命名后创建角色；创建过程自动生成角色提示词。
...
`manifest.json` 的 `runtime.python` 是唯一的环境声明：平台创建 venv、按 `python/requirements.lock` 安装 PyTorch、faster-whisper 与 CosyVoice 推理依赖；`bootstrap.sh` 是不受产品约束的兜底脚本，本 App 用它浅克隆 CosyVoice 官方仓库。Whisper 权重从 Hugging Face（Systran/faster-whisper-*）下载，CosyVoice2-0.5B 权重从 Hugging Face（FunAudioLLM/CosyVoice2-0.5B）下载，均以单文件进度输出到任务日志。音频抽取、16k 参考音归一化都要求本机 `ffmpeg` 已可执行；安装、下载、转写、角色准备和合成均作为可取消任务运行，进度和错误显示在界面和项目事件流中，错误可直接交给右侧 Codex 处理。

## 数据边界

| 数据 | 保存位置 |
| --- | --- |
| CosyVoice 官方代码、Whisper 与 CosyVoice2 权重 | `~/.recut/models/audio-studio/` |
