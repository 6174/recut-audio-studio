...
- 应用进入时先自动调用 `audio.status -> audio.prepare`；运行依赖未就绪时不能开放工作台。转写前确认用户选择的 Whisper 模型已经下载；配音前确认 CosyVoice2 权重已下载且至少存在一个已完成的声音角色。
...
- 创建声音角色时，参考音频会被裁剪到前 30 秒，并以已安装的 Whisper 模型自动转写生成角色提示词（`audio.character.create` 异步提交、`audio.character.complete` 读取结果）。没有提示词的角色不能用于 `audio.synthesize`。
