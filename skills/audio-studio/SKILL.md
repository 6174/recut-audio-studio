...
- `audio.synthesize` 只提交 Job，worker 合成后必须经过 Qwen3-ASR 回读，且双向文本相似度至少 `0.85`；未通过的 WAV 会删除，`audio.synthesis.complete` 不会暴露它。用户明确点击保存后（或平台把本机 TTS 选为语音默认路由时，`recut.speech.generate` 的 local-audio 路由经 daemon 桥调用），允许 `audio.save` 把已验收配音导入素材库。
- 模型任务的执行日志必须持续可见：CosyVoice worker 实时输出代码准备、权重加载、参考音编码、推理分片和 WAV 写入；主 worker 在底层模型连续 8 秒无输出时发送心跳。ASR 只在完整 WAV 写出后执行一次回读验收，不在合成过程中重复运行。
