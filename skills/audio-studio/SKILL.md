...
- 应用进入时先自动调用 `audio.status -> audio.prepare`；运行依赖未就绪时不能开放工作台。Agent 会话内 `audio.status` 只读一次：之后的就绪判断（转写模型已下载、配音引擎 venv/权重就绪）都基于已知状态，只有环境实际报错才重读，不为每次转写或配音预检。转写前确认用户选择的 Whisper 或 Qwen3-ASR 模型已经下载；模型下载的 `source` 只能是 `automatic`、`huggingface` 或 `modelscope`，自动模式先尝试 Hugging Face 再回退 ModelScope；配音引擎要求：CosyVoice 需要 CosyVoice 专属官方 venv、CosyVoice2 权重与 Qwen3-ASR 0.6B 回读器；VoxCPM 需要 `engines.voxcpm.runtime` 与其某版本权重（`voxcpm2` 约 5.0GB / `voxcpm1.5` 约 2.0GB / `voxcpm-0.5b` 约 1.6GB）。专属 venv 未就绪时，用 `audio.prepare { target: "cosyvoice" | "voxcpm" }` 定向修复单个环境（缺省 `all` 为全量准备，含主 venv）；UI 侧这些动作集中在右上角「模型与环境」设置面板。未选择角色时，CosyVoice 使用官方默认声音，VoxCPM2 使用 Voice Design 默认音（无参考音），VoxCPM1.5/0.5B 必须提供声音角色参考音。
...
- 所有耗时操作（`audio.prepare` / `audio.install` / `audio.transcribe` / `audio.character.create` / `audio.character.design` / `audio.synthesize`）都是异步提交、返回 taskId；**并发模型**（rfc/2026-09-03-task-queue-and-parallelism.md）：推理类（转写/建角色/设计/配音）单槽 FIFO 串行，环境准备单槽串行，模型下载不限并行。提交永不报错拒绝：槽位空闲立即派发（返回的 `job` 非空），被占时 `job=null`（已排队，槽位释放后自动开始）——**不要把 `job=null` 当作失败重试**，用 `audio.status` 的 `tasks` 清单（或 `audio.tasks.list`）观察 queued/running 状态，用 `recut.job.wait`（job 非空时）/轮询 `audio.status` 等待终态；失败用 `recut.job.logs` 诊断、`recut.job.cancel` 或 `audio.task.cancel`（支持排队中）取消。`audio.status` 的 activeJob 只用于确认最新在途任务归属；`audio.task.cancel` 的 queued 任务直接落 cancelled。
...
- 平台 `ctx.python` venv 承载 Qwen/Whisper ASR，`requirements.lock` 固定完整依赖闭包；`bootstrap.py` 在同级创建 CosyVoice 专属 venv 与 VoxCPM 专属 venv，固定安装各自锁文件的依赖闭包、固定 CosyVoice/Matcha-TTS 源码提交，并执行 `pip check` 与 worker 版本自检。三者不可合并，因为 Qwen3-ASR、CosyVoice2 与 VoxCPM 对 torch/transformers 版本存在不可调和的约束。bootstrap 支持 `--target all|cosyvoice|voxcpm` 定向准备；VoxCPM 运行环境在 `prepare` 中尽力准备（失败不阻断），`audio.status` 的 `engines.voxcpm.runtime` 暴露状态。注意：环境准备（venv/pip 依赖）与模型权重下载（`audio.install`）是两件事，`bootstrap` 永不下载模型权重；已下载的权重（`~/.recut/models/audio-studio/`）在任何准备流程中都不应被重新下载。
